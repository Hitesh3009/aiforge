...
function Notification({ email, message, onClose, statusCode,isLogin,userName }) {
    ...
    const textColor = isError === 409 || isError === 404 || isError === 401 ? 'text-red-700' : 'text-green-700';
    const bgColor = isError === 409 || isError === 404 || isError === 401 ? 'bg-red-100 border-red-400' : 'bg-green-100 border-green-400';
    return (
        <div className="fixed top-4 right-4 z-50">
            <div className={`border ${bgColor} ${textColor} px-4 py-3 rounded shadow-lg flex items-center justify-between gap-4`}>
                {! isLogin ?<span>{message} <b>{email}</b></span>:<span>{message} <b>{userName}</b></span>}
