...
export async function POST(req) {
    ...
    try {
        if (session) {
            const { prompt,referenceImg,targetImg } = await req.json();
            const apiKey = process.env.FREEPIK_API_KEY
            //console.log(apiKey);

            const url = 'https://api.freepik.com/v1/ai/image-style-transfer';
            const options = {
                method: 'POST',
                headers: { 'x-freepik-api-key': apiKey, 'Content-Type': 'application/json' },
                body: JSON.stringify({ "prompt": prompt,"image": targetImg,"reference_image":referenceImg,"flavor":"detaily", engine : "real" })
            };
